...
- **One-machine mode writes nothing to disk.** Captured cookies, tokens, and web storage exist only in memory for the duration of a run, and are discarded when the process exits. Nothing about the session is recorded locally. To record and compare results, use the hosted self-assessment tool.
- **Two-machine mode is the exception, by necessity.** Exporting a session writes live material (cookies, tokens, storage) to a `relock-export-<domain>.json` file (on your Desktop, or the launch directory) so it can be moved to the second machine; the harness prints the exact path. That file is effectively full access to the account. Use it **only with throwaway test accounts**, move it over a trusted channel, and delete it after import (the import flow lists the files it finds and offers to delete the one you used). It is git-ignored so it cannot be committed.
